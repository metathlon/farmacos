...
function cargar_ayuda(pmt) {
		theObject = document.getElementById('vhelp');
		urlDestino="lx_help.php?id_help=" + pmt;
		ajax=objetoAjax();
...
function cargar_farmacos_pathway_old(pmt1) {
		//alert(inputString);
...
function cambia_estado_pathway(pmt1) {
		var nombre_estado_path='chk_pw_' +  pmt1;
		theObject2 = document.getElementById(nombre_estado_path);
			if (theObject2.checked==true){
			var estado_path=1;
			}else{
			var estado_path=0;
			}
		urlDestino="functions_custom.php?op=cepw&id_pathway="+ pmt1 +"&pw_activo=" + estado_path;
		ajax=objetoAjax();
		ajax.open("POST", urlDestino,true);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4) {
			mostrar_farmacos_pathway(0);
			}
		}
		ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		ajax.send(null)      
...
function mostrar_farmacos_pathway(actualiza_pathway) {
        if (actualiza_pathway==1){
		
			theObject3 = window.opener.document.getElementById('cpathways');
			 theObject3.innerHTML ='';
			 urlDestino3="lx_pw.php";
			ajax3=objetoAjax();
			ajax3.open("POST", urlDestino3,true);
			ajax3.onreadystatechange=function() {
				if (ajax3.readyState==4) {
				theObject3.innerHTML = ajax3.responseText;
				}
			}
