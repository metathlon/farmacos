...
$excel=new ExcelWriter("export_excel/fda_drugs.xls");
if($excel==false) {    
echo $excel->error;
}
			//cabecera
			...
			while (list ($cod_indicacion,$des_indicacion) = each ($array_indicaciones_cabecera)){
			$excel->writeCol("");
			}
			reset($array_indicaciones_cabecera);
			...
			$excel->writeCol("<b>More Information</b>");
			while (list ($cod_link, $des_link) = each ($array_links_cabecera)){
			...
			while (list ($cod_indicacion,$des_indicacion) = each ($array_indicaciones_cabecera)){
			$excel->writeCol("<b>".$des_indicacion."</b>");
			}
			reset($array_indicaciones_cabecera);
			while (list ($cod_tox, $des_tox) = each ($array_toxicidades_cabecera)){
			...
			reset($array_toxicidades_cabecera);
			$excel->writeCol("<b>CLASIFICATION ADVERS REACTIONS IN CLINICAL TRIALS</b>");
			$excel->writeRow();
	...
	$numero_farmacos=mysql_num_rows($resultado);
		if($numero_farmacos == 0) {
				...
				while(list($id_farmaco,$nombre_farmaco,$observaciones_farmaco,$clasificacion_toxicidad_farmaco)= mysql_fetch_row($resultado)){
					...
					$excel->writeCol($observaciones_farmaco);	
					while (list ($cod_link, $des_link) = each ($array_links_cabecera)){
					...
					reset($array_links_cabecera);
					while (list ($cod_indicacion,$des_indicacion) = each ($array_indicaciones_cabecera)){
						if (array_key_exists($cod_indicacion,$array_indicaciones_farmaco)){
							if ($array_indicaciones_farmaco[$cod_indicacion]==1){
							$excel->writeCol("<b>1*</b>");		
							}else{
							$excel->writeCol("1");		
							}			
						}else{
					...
					$excel->writeCol($clasificacion_toxicidad_farmaco);
...
$excel->writeRow();
$excel->close();
header("location:export_excel/fda_drugs.xls");
?>